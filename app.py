...
model = load_model(
    'D:\Coding\prediksi_kanker_paruparu\model\my_model.h5')

# Load the scaler object used during training
scaler = StandardScaler()
scaler_file = 'D:\Coding\prediksi_kanker_paruparu\model\scaler.pkl'
...
@app.route('/')
def home():
    return render_template('landing.html', favicon_url=url_for('static', filename='images/favicon.png'))
...
def results():
    if request.method == 'POST':
        ...
        if predicted_classes[0] == 0:
            outcome = 'Risiko Kanker Paru-paru Rendah (Low)'
        elif predicted_classes[0] == 1:
            outcome = 'Risiko Kanker Paru-paru Sedang (Medium)'
        else:
            outcome = 'Risiko Kanker Paru-paru Tinggi (High)'
